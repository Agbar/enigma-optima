...
TEST( set_ukw, H_M3_ukw_A ) {
    Key k;
    k.ukwnum.type = (ukw_type_enum)-1;
    EXPECT_TRUE( set_ukw( &k, "A", ModelType_t::EnigmaModel_H ) );
    EXPECT_EQ( k.ukwnum.type, UkwType_A );
}
...
TEST( set_ukw, H_M3_ukw_B ) {
    ...
    EXPECT_EQ( k.ukwnum.type, UkwType_B );
}
...
TEST( set_ukw, H_M3_ukw_C ) {
    ...
    EXPECT_EQ( k.ukwnum.type, UkwType_C );
}
...
TEST( sek_ukw, M4_ukw_B ) {
    ...
    EXPECT_EQ( k.ukwnum.type, UkwType_B_Thin );
}
...
TEST( sek_ukw, M4_ukw_C ) {
    ...
    EXPECT_EQ( k.ukwnum.type, UkwType_C_Thin );
}
